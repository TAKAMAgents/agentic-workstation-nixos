...
assert_supported_install_os() {
  ...
  if [[ "$os_id" != "ubuntu" ]]; then
    cat >&2 <<'EOF'
error: the mutating Bash installer supports Ubuntu only.

This host is not Ubuntu. On NixOS, use the flake module instead:

  nix run .#nixos-module

You can still inspect plans without mutating the host:

  ./install-agentic-tools.sh --profile coding-agent --json-plan
  nix run .#plan -- --profile coding-agent
EOF
    exit 1
  fi
}
